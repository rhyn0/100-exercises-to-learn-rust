...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        ...
        Err(TicketNewError::DescriptionError { description }) => Ticket {
            title,
...
impl Ticket {
    pub fn new(
        ...
        if title.len() > 50 {
            return Err(TicketNewError::TitleError {
                description: "Title cannot be longer than 50 characters".to_string(),
            });
        }
        ...
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionError {
                description: "Description cannot be longer than 500 characters".to_string(),
            });
        }
